...
fn run_test_display(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
...
fn run_send(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Spawn camera thread for ACK detection
    ...
    thread::spawn(move || {
        let mut camera = match camera::CameraCapture::new(cam_idx, cam_w, cam_h) {
            Ok(c) => c,
            Err(e) => {
                log::warn!("camera init failed: {e} — using timeout fallback");
                // Fallback: just advance frames on a timer
        ...
        loop {
            ...
            if let Some(quad) = detection::detect_frame(&img) {
                ...
                let dst = quad.corners;
                if let Some(h) = detection::Homography::from_points(&src, &dst) {
                    let (indices, _confs) =
                        detection::sample_cells(&img, &h, &palette, 50, 50);
                    if let Some(decoded) = decoder::decode_grid(&indices, 50, 50) {
                        if decoded.header.frame_type == protocol::FrameType::Ack {
                            let ack_id = decoded.header.frame_id;
                            log::info!("received ACK for frame {ack_id}");
                            if ctrl.send(SendCtrl::NextFrame).is_err() {
                                break;
                            }
                        }
...
fn run_receive(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    let output_path = cli
        .output
        .clone()
        .unwrap_or_else(|| "received_output.bin".to_string());

    log::info!("starting receive mode, output will go to {output_path}");
                        ...
                        if header.frame_type == protocol::FrameType::Data {
                            ...
                            if is_new {
...

                                {
                                    ...
                                    if s.is_empty() {
                                        *s = vec![None; header.total_shards as usize];
                                    }
                                    if (header.shard_index as usize) < s.len() {
                                        s[header.shard_index as usize] =
                                            Some(decoded.payload.clone());
                                    }
                                }
                                ...
                                if complete {
                                    ...
                                    let all_vecs: Vec<Vec<u8>> = {
                                        let s = shards.lock().unwrap();
                                        s.iter()
                                            .map(|sh| sh.clone().unwrap_or_default())
                                            .collect()
                                    };
